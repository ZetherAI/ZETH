...
import { House, Swords, List, Handshake, FlaskConical, ChevronRight } from "lucide-react";
...
const Icon = ({ i, color }: { i: number; color: string }) => (
	<>
		...
		{i === 2 && <RiCustomerService2Line style={{ color }} className="text-2xl" />}
		...
		<div className="relative h-full">
			...
			<div
				className={`relative backdrop-blur w-full h-full py-4 md:py-5 overflow-hidden transition-500 bg-black/30 ${
					showFull ? "w-full md:max-w-fit lg:min-w-[250px] px-4 md:px-5" : "max-w-fit px-2"
				}`}
			...
			>
				...
				<MotionDiv
					...
					className={`flex flex-col gap-3 relative ${!showFull ? "items-center" : ""}`}
				>
						...
						<motion.div key={i} variants={variants.slideInBottom}>
							<Link
								className={`uppercase font-semibold text-white p-[10px] flex-v-center group border  rounded-lg hover:border-white hover:bg-white/10 w-full transition-500 ${
									pathname === `/${link}` ? "border-white bg-white/10" : "border-transparent"
								}`}
								href={`/${link}`}
								// scroll={false}
							>
